...
export default function InstituicoesPage() {
  ...
  const handleDelete = async (id: number) => {
    if (typeof window !== 'undefined' && window.confirm('Tem certeza que deseja excluir esta instituição? Concursos e questões associadas poderão ser afetados.')) {
      setLocalLoading(true);
      try {
              ...
              <li key={inst.id} className="hover:bg-gray-50 transition-colors duration-150">
                <div className="px-4 py-4 sm:px-6 flex justify-between items-center gap-4 text-sans">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-indigo-600 truncate mb-1" title={inst.nome}>
                      {inst.nome}
                    </div>
