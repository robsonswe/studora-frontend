        ...
        {loading ? (
          ...
          disciplinas.map(d => {
            ...
            return (
              <button
                key={d.id}
                onClick={() => router.push(`/disciplinas/${d.id}`)}
...
export default function DisciplinasPage() {
  return (
    <div className="min-h-screen pb-24 bg-slate-50 font-sans text-slate-800 antialiased">
      <Suspense fallback={
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}
        </div>
      }>
        <DisciplinasContent />
